...
复杂长文默认分段预览，公式、代码块和表格不会拆开，发布仍为完整文章。切回编辑或重新检查时会卸载旧预览；需要查看整页主题时，可手动打开“完整主题预览”。单个内容块过大时暂停自动加载，避免立即拖垮编辑页面。

预览诊断记录在 `.blog-studio/preview-diagnostics.jsonl`（自动轮转），包含构建阶段、耗时、页面大小和 iframe 加载事件，不记录文章正文或凭据。浏览器整个渲染进程崩溃时可能无法上报最后事件，仍需结合浏览器崩溃报告排查。
